...
const LanguageToggle: React.FC<{ current: Language; onToggle: (lang: Language) => void; isScrolled: boolean; }> = ({ current, onToggle, isScrolled }) => {
  // When not scrolled (on hero image), use a light glass effect.
  // When scrolled (on light page bg), use a dark glass effect for contrast.
  ...
  const buttonThemeClasses = (isActive: boolean) => {
    if (isActive) {
      return 'bg-white text-[var(--text-dark)] shadow'; // Active is always the same
    }
    // Inactive buttons adapt their text color
    return isScrolled
...
const Navbar: React.FC<{ setPage: (page: Page) => void; lang: Language; setLang: (lang: Language) => void; t: (key: string) => string; isScrolled: boolean; }> = ({ setPage, lang, setLang, t, isScrolled }) => (
    <header className="fixed top-0 left-0 w-full z-50 p-6">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        {/* Title also changes color for visibility */}
...
export const HomePage: React.FC<HomePageProps> = ({ setPage, lang, setLang, t, config }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Set true if user has scrolled more than 10px from the top
